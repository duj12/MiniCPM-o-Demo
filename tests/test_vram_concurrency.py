...
def extract_audio_frames(video_path, max_frames=0):
    """从视频提取 PCM 音频并按 100ms 切帧。
    返回 (pcm_frames, total_seconds)
    """
    r = subprocess.run(
...
def extract_video_frames(video_path, max_n=3):
    """从视频均匀提取 JPEG 帧 base64。"""
...
async def run_duplex_session(idx, gateway, audio_frames, video_frames=None,
                              text_prompt=None, frame_gap=0.1, timeout=120):
    """
    一路全双工流式会话。
    逐帧发送音频（首帧附带 video_frames/text），接收模型响应。
    返回 (idx, ok, conn_ms, total_ms, info)
    """
    t0 = time.perf_counter()
    try:
        ws = await websockets.connect(gateway, max_size=128*1024*1024, ssl=SSL_CTX)
...
async def main():
    ...
    parser.add_argument("--duplex-frames", type=int, default=10,
                        help="全双工模式每路音频帧数 (默认 10 = 1s, 视频源建议 5-10)")
    ...
    if mode == "duplex":
        print("  - 每路帧数: %d (%.1fs)" % (args.duplex_frames,
              args.duplex_frames * FRAME_MS / 1000))
        if args.video:
    ...
    if mode == "video":
        ...
        if not os.path.exists(video_path):
            print("\n  视频文件不存在: %s" % video_path)
            return
        print("\n  提取视频音频+帧: %s" % video_path)
        duplex_audio_frames, total_s = extract_audio_frames(
            video_path, max_frames=args.duplex_frames)
        duplex_video_frames = extract_video_frames(video_path, max_n=3)
        print("  音频: %d 帧 (%.1fs), 视频帧: %d 张\n" %
              (len(duplex_audio_frames), total_s, len(duplex_video_frames)))
        ...
        async def start_one(idx):
            ...
            if mode == "text":
                ...
                return await run_duplex_session(
                    ...
                    video_frames=duplex_video_frames,
