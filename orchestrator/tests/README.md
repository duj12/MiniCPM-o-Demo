...
对齐时延迟估计器能精确恢复真值（误差 **0 采样**）。

⚠️ **这张表要在"落位已由浏览器承诺"的前提下读**。D 里只剩设备固有的
声学延迟 + 音频 I/O 缓冲 —— 网络往返、浏览器主线程抖动、mic 在途积压
都被移出了 D（见 `orchestrator/README.md` 的「参考轨的播出时刻必须由
浏览器承诺」）。改动之前这些都在 D 里，且**逐句变化**（实测同一次会话内
第二句的预测误差比第一句大 888ms），所以那时候固定常量 D 根本无从谈起。

⚠️ **估计器已从运行时撤下**：`AcousticDelayTracker` 现在住在
`orchestrator/tools/delay_estimate.py`，只被**离线**的
`tests/measure_delay.py` 使用。运行时自适应在真机上收敛不了，且一旦被
噪声假峰钉死就会永久失效。
...
`test_duplex_sim.py` 用假浏览器合成真实回声闭环，两种落位模式对照：

| 模式 | 第 1 句落位偏差 | 第 2 句落位偏差 | 散度 |
|---|---|---|---|
| `--anchor armed`（承诺落位） | −59 采样（−3.7ms） | −59 采样（−3.7ms） | **0 采样** |
| `--anchor legacy`（服务端预测） | −5104 采样（−319ms） | −19312 采样（−1207ms） | **888ms** |

两种模式跑的是**同一份服务端代码**，差别只在浏览器是否承诺起播时刻。

两个结论：

  ① armed 的偏差是一个**固定的块相位**（−3.7ms），不是误差 —— 它会被
     并进 D 里一起被 AEC 消掉。D 只要测一次就是个常量。
  ② legacy 的偏差**逐句变化**（本例差 888ms）。AEC 的容忍窗只有 ±5ms，
     固定常量 D 吸收不了变化量 —— 这就是"第一句能消、后面失效"的机制。

⚠️ 上面那个 −59 采样来自仿真器自己的建模（采集块的 ctx 时刻比块内首采样
晚一个渲染量子相位），不是链路误差；真机上它由 measurement 出来的 D 吸收。

`test_duplex_sim.py` 还暴露了一个**打断路径的真实缺陷**：`_interrupt_current`
原先用 `clock.now()`（已 ingest 的采样数）做临时截断点，它系统性落后真实
播放位置 —— 实测打断后参考轨比实际播出量**少 6421 采样（401ms）**，那段
回声完全没有参考。现在改用锚点估算 + 300ms 余量（`resize` 只能缩短，
所以"宁多勿少"）。
