...
- GPU 0 空闲显存：~9,067 MiB（含 TTS 模型 + 其他服务占用）
...
**单轮视频并发测试（121.mp4, 9.6MB, turn_based, Q4_K_M, 流式模式）：**

| 并发路数 | 通过 | TTFT(首字) | 生成耗时 | 生成速度 | 峰值 VRAM | GPU 利用率 |
|:-------:|:---:|:----------:|:--------:|:--------:|:---------:|:---------:|
| 1 | ✅ | **2.5s** | 3.1s | 44 ch/s | 10,905 MiB | 22% |
| 2 | ✅ | 2.7-2.8s | 3.2-4.6s | 38-79 ch/s | 12,709 MiB | 93% |
| 3 | ✅ | 2.3-2.7s | 3.4-4.6s | 63-101 ch/s | 14,509 MiB | 96% |
| **4** | **✅** | **2.6-2.7s** | **3.0-5.8s** | **49-85 ch/s** | **14,519 MiB** | **100%** |

TTFT（Time To First Token）指从发送视频到模型输出第一个字的时间，约 2.5s，其中主要耗时在视频帧编码和 LLM prefill。生成速度为后续逐 token 输出速度，4 路并发下稳定在 50-85 ch/s。

**全双工流式视频并发测试（121+pad.mp4, 73MB, 模拟实时采集）：**

模拟真实摄像头+麦克风输入：音频按 100ms 帧发送，视频按 5fps 在对应音频帧时间点附带 JPEG 帧。每路发送 30 帧（3 秒）。

| 并发路数 | 通过 | wall time | 推理时峰值 VRAM | GPU 利用率 |
|:-------:|:---:|:---------:|:--------------:|:---------:|
| 1 | ✅ | 16.3s | 10,871 MiB | 89% |
| 2 | ✅ | 18.7s | 12,677 MiB | 86% |
| 3 | ✅ | 28.7s | 14,483 MiB | 100% |
| **4** | **✅** | **34.2s** | **16,291 MiB** | **94%** |
...
**模型精度对比测试（全双工视频 4 路，121+pad.mp4, 30帧/路, 5fps）：**

| 模型 | 精度 | 文件大小 | 基线显存 | 4 路全双工 | 最大并发 | 推理时峰值 VRAM | GPU 利用率 |
|:----|:----:|:--------:|:--------:|:----------:|:--------:|:--------------:|:----------:|
| `Q4_K_M` | 4-bit | 4.7 GB | 9,067 MiB | ✅ | **4 路** | **16,291 MiB** | 89-100% |
| `Q8_0` | 8-bit | 8.2 GB | 10,655 MiB | ❌ 3/4 | **3 路** | **23,928 MiB** | 96-100% |
| `F16` | 16-bit | 16 GB | 17,421 MiB | ❌ 2/3 | **2 路** | **22,673 MiB** | 99% |

> 注：Q8_0 和 F16 的峰值数据通过实时监控（0.3s 间隔采样）在并发推理期间采集，反映真实的显存天花板。Q4_K_M 的数据为实时监控采集。Q8_0 3 路峰值 23,928 MiB 已逼近 24GB 物理上限，F16 3 路将超过 24GB。

F16 模型因基线显存较高（17.4GB），仅能支持 2 路全双工视频并发，3 路时显存耗尽。
