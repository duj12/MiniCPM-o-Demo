...
|:----|:----:|:--------:|:--------:|:----------:|:--------:|:--------------:|:----------:|
| `Q4_K_M` | 4-bit | 4.7 GB | 9,161 MiB | ✅ | **4 路** | 10,945 MiB | — |
| `Q8_0` | 8-bit | 8.2 GB | 10,655 MiB | ❌ 3/4 | **3 路** | **23,928 MiB** | 96-100% |
| `F16` | 16-bit | 16 GB | 17,421 MiB | ❌ 2/3 | **2 路** | **22,673 MiB** | 99% |

> 注：Q8_0 和 F16 的峰值数据通过实时监控（0.3s 间隔采样）在并发推理期间采集，反映真实的显存天花板。Q4_K_M 的数据为优化前采集。Q8_0 3 路峰值 23,928 MiB 已逼近 24GB 物理上限，F16 3 路将超过 24GB。
